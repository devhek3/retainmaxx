...
export default function SavedVideoRow({ onPress, video }) {
  return (
    <Pressable
      accessibilityHint="Opens saved video details"
      accessibilityLabel={`${video.title}, ${video.topic}, ${video.source}`}
    ...
    >
      <ImageBackground imageStyle={styles.image} source={video.image} style={styles.thumbnail}>
        ...
        <AppText style={styles.overlay}>{video.overlay}</AppText>
      </ImageBackground>
      <View style={styles.details}>
        <AppText numberOfLines={2} style={styles.title}>{video.title}</AppText>
        <View style={styles.metadata}>
          <View style={styles.dot} />
          <AppText numberOfLines={1} style={styles.metaText}>{video.topic}</AppText>
          <AppText style={styles.separator}>•</AppText>
          <AppText numberOfLines={1} style={styles.metaText}>{video.source}</AppText>
        </View>
      </View>
      <View accessibilityElementsHidden style={styles.moreButton}>
...
const styles = StyleSheet.create({
  row: { alignItems: 'center', borderRadius: 12, flexDirection: 'row', minHeight: 90 },
  ...
  overlay: { color: colors.surface, fontSize: 11, fontWeight: fontWeights.bold, letterSpacing: 0.1, lineHeight: 13, maxWidth: 86, textTransform: 'uppercase', textShadowColor: 'rgba(0, 0, 0, 0.55)', textShadowOffset: { height: 1, width: 0 }, textShadowRadius: 2 },
  details: { flex: 1, justifyContent: 'center', minWidth: 0, paddingLeft: 18, paddingRight: 3 },
  title: { color: '#0E1118', fontSize: 17, fontWeight: fontWeights.medium, letterSpacing: -0.4, lineHeight: 21 },
